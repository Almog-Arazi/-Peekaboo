...
ellipse_center = (960, 540)  # Center of the screen
ellipse_axes = (400, 540)  # Axes of the ellipse (width, height)
...
# Reposition left eye corner to be the origin12
...
not_looking_time = None  # Initialize time tracking for non-looking period
nudge_count = 0  # Initialize nudge counter

mirror = MirrorController(servo_pin)
mirror.setup()
mirror.init_mirror_pos()

def calculate_gaze_direction(lx_score, ly_score, rx_score, ry_score):
    avg_x_score = (lx_score + rx_score) / 2 - 0.5
    avg_y_score = (ly_score + ry_score) / 2 - 0.5

    if avg_x_score < -0.1:
        return "Looking Away"
    elif avg_x_score > 0.05:
        return "Looking Away"
    elif avg_y_score > 0.05:
        return "Looking Away"
    elif avg_y_score < -0.1:
        return "Looking Away"
    return "Looking The mirror"

while cap.isOpened():
    success, img = cap.read()

    if not success:
        continue  # If no frame is read, skip the iteration
    ...
    img_h, img_w, img_c = img.shape  # Define img_w and img_h based on the current frame's shape

    # Draw the elliptical mask for visualization
    cv2.ellipse(img, ellipse_center, ellipse_axes, 0, 0, 360, (0, 0, 255), 2)

    person_looking = False
    ...
    if not results.multi_face_landmarks:
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Check if the face is within the elliptical mask
            nose_tip = face_2d[1]  # Nose tip is landmark 1
            if ((nose_tip[0] - ellipse_center[0]) / ellipse_axes[0])**2 + ((nose_tip[1] - ellipse_center[1]) / ellipse_axes[1])**2 > 1:
                continue  # Skip this face if the nose tip is outside the ellipse
            ...
            gaze_direction = calculate_gaze_direction(lx_score, ly_score, rx_score, ry_score)

            if gaze_direction != "Looking The mirror":
                if not_looking_time is None:
                    not_looking_time = time.time()  # Start the timer when not looking at the mirror
                    nudge_count = 0  # Reset nudge counter
                elapsed_time = time.time() - not_looking_time
                if elapsed_time > 5 and nudge_count < 2:
                    cv2.putText(img, f"Not looking at mirror for 5 sec ({int(elapsed_time)} sec)", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    mirror.nudge()
                    nudge_count += 1    
                cv2.putText(img, f"Timer: {int(elapsed_time)} sec", (50, img_h - 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            else:
                not_looking_time = None  # Reset the timer if looking at the mirror
                nudge_count = 0  # Reset nudge counter

            # Display gaze direction
            cv2.putText(img, gaze_direction, (450, 50), cv2.FONT_HERSHEY_DUPLEX, 2, (0, 255, 0), 2)
